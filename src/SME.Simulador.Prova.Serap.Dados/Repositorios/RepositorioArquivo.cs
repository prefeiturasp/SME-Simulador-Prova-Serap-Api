...
namespace SME.Simulador.Prova.Serap.Dados
{
    public class RepositorioArquivo : IRepositorioArquivo
    {
        ...
        {
            var query = @"select f.[Path] Caminho, f.Id
            ...
            return await gestaoAvaliacaoContexto.Conexao.QueryAsync<ArquivoDto>(query, new { questaoId }, transaction: gestaoAvaliacaoContexto.Transacao);
        }
        ...
        public async Task<IEnumerable<VideoDto>> ObterVideosPorQuestaoId(long questaoId)
        {
            ...
            return await gestaoAvaliacaoContexto.Conexao.QueryAsync<VideoDto>(query, new { questaoId }, transaction: gestaoAvaliacaoContexto.Transacao);
        }
    }
}
